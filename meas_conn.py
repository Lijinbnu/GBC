...
def getdata(vol,mask):
    # load volume and mask in memory 
    vol_data = vol.get_data()
    mask_data = mask.get_data()
    affine = vol.get_affine()

    # get volume time course matrix
    maskts = vol_data[mask_data.astype(np.bool),:]
    # get volume coordinate
    i,j,k = np.nonzero(mask_data)
    coord = np.array([i,j,k]).T

    return maskts,coord,affine
